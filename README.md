...
- **Workspace save/load** — save selection, expansion, ignore patterns, edge highlights, folder colors, edges type, and layout into the cruise JSON under `dependency-cruiser-json-viewer` (**Workspace: Save**). Both **Cruise Result: Load JSON** (when the file has workspace settings) and **Workspace: Load Settings** always clear the current view and replace selection, expansion, the dependencies panel path, ignore patterns, edge highlights, folder colors, edges type, and layout from the file, dropping entries that no longer match the relevant cruise data (e.g. references to files or dependencies that no longer exist). With the CLI, pass `--workspace-settings <path>` to apply a saved workspace once on startup (same as **Workspace: Load Settings**) so you do not have to load it manually every time.
