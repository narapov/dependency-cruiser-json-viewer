...
import type { QuickOpenResultItem } from '../../types'
...
function searchItem(key: string, name = key.split('/').pop() ?? key): QuickOpenResultItem {
  return {
    key,
    name,
    isFolder: false,
...
describe('searchPaths', () => {
  ...
  it('filters by file name and path', () => {
    expect(searchPaths(items, 'app').map((item) => item.key)).toEqual([
      'src/components/App.tsx',
    ])
  ...
  it('is case-insensitive', () => {
    expect(searchPaths(items, 'BUTTON').map((item) => item.key)).toEqual([
      'src/components/Button.tsx',
    ])
  })
  ...
  it('matches scattered query characters across the full path', () => {
    ...
    expect(
      searchPaths(fuzzyItems, 'srconteSearque').map((item) => item.key),
