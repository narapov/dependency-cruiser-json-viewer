...
describe('getDependencyEdgePath', () => {
  ...
  it('bulges reverse same-Y edges for straight style', () => {
    vi.mocked(getStraightPath).mockClear();

    const [path, , labelY] = getDependencyEdgePath({ ...reverseSameY, edgeStyle: 'straight' });
  ...
  it('does not bulge reverse same-Y edges for orthogonal style', () => {
    vi.mocked(getSmoothStepPath).mockClear();

    const result = getDependencyEdgePath({ ...reverseSameY, edgeStyle: 'orthogonal' });
  ...
  it('delegates forward same-Y edges to getStraightPath for straight style', () => {
    vi.mocked(getStraightPath).mockClear();

    const result = getDependencyEdgePath({ ...forwardSameY, edgeStyle: 'straight' });
  ...
  it('delegates different-Y edges to getSmoothStepPath for orthogonal style', () => {
    vi.mocked(getSmoothStepPath).mockClear();
