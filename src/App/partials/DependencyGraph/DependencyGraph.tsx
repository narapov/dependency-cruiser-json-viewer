...
import type { FolderBaseColor, GraphEdgeStyle } from '@/domain';
import { downloadTextFile, openGraphvizOnline, useResolvedColorMode } from '@/Shared';

import { EdgeStyleProvider, GraphActionsProvider } from './contexts';
...
import { EdgeStylePickerDialog } from './partials/EdgeStylePickerDialog';
...
interface DependencyGraphInnerProps {
  ...
  onEdgeStyleChange: (value: GraphEdgeStyle) => void;
  onOpenEdgeStylePicker: () => void;
}
...
function DependencyGraphInner(props: DependencyGraphInnerProps) {
  ...
  useImperativeHandle(imperativeRef, () => {
    ...
    return {
      ...
      openEdgeStylePicker: onOpenEdgeStylePicker,
      getLayoutState: () => ({
        autoLayoutOnly,
        edgeStyle,
      ...
      setLayoutState: state => {
        ...
        onEdgeStyleChange(state.edgeStyle);
...
export function DependencyGraph(props: DependencyGraphProps) {
  ...
  const [edgeStyle, setEdgeStyle] = useState<GraphEdgeStyle>('bezier');
    ...
    <div className={clsx(styles.container, autoLayoutOnly && styles.layoutLocked)}>
      <ReactFlowProvider>
        <DependencyGraphInner
          ...
          onOpenEdgeStylePicker={() => setEdgeStylePickerOpen(true)}
          {...rest}
        />
      </ReactFlowProvider>
      <EdgeStylePickerDialog
        open={edgeStylePickerOpen}
        edgeStyle={edgeStyle}
        onEdgeStyleChange={setEdgeStyle}
        onClose={() => setEdgeStylePickerOpen(false)}
      />
    </div>
  );
}
