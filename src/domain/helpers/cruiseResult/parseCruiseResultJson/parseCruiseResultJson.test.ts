...
import {
  ...
  validateCruiseResult,
} from './parseCruiseResultJson'
...
describe('parseCruiseResultJson', () => {
  ...
  it('throws invalidFormat when modules is missing', () => {
    expect(() => parseCruiseResultJson(JSON.stringify({ summary: {} }))).toThrow(
      CruiseResultParseError,
    )
    try {
  ...
  it('throws invalidFormat when a module has no source', () => {
    expect(() =>
      parseCruiseResultJson(JSON.stringify({ modules: [{ dependencies: [] }] })),
...
describe('validateCruiseResult', () => {
  ...
  it('throws invalidFormat when a module has no source', () => {
    expect(() => validateCruiseResult({ modules: [{ dependencies: [] }] })).toThrow(
      CruiseResultParseError,
    )
  })
})
