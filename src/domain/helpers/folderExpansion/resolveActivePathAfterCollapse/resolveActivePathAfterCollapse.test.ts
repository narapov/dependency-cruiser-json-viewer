...
import { resolveActivePathAfterCollapse } from './resolveActivePathAfterCollapse'
...
describe('resolveActivePathAfterCollapse', () => {
  ...
  it('picks deepest collapsed ancestor on collapse all', () => {
    expect(resolveActivePathAfterCollapse('src/foo/bar.ts', ['src', 'src/foo'])).toBe('src/foo')
  })
